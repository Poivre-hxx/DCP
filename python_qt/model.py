...
def nearest_neighbor(src, dst):
    inner = -2 * torch.matmul(src.transpose(1, 0), dst)
    distances = -torch.sum(src ** 2, dim=0, keepdim=True).transpose(1, 0) - inner - torch.sum(dst ** 2, dim=0, keepdim=True)
    distances, indices = distances.topk(1, dim=-1)
...
class DGCNN(nn.Module):
    def __init__(self, emb_dims):
        ...
        self.conv5 = nn.Conv2d(512, emb_dims, 1)
